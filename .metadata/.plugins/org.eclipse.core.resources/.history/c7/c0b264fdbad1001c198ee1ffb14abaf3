...
<div class="col-lg-9">
	...
	<div class="row">
		<div class="col">
			<button class="btn btn-danger btn-sm form-control font-weight-bold"
				onclick="fSetStatus(-1)">
				<span class="fas fa-trash"></span> HỦY
			</button>
		</div>
		<div class="col">
			<button class="btn btn-success btn-sm form-control font-weight-bold"
			onclick="fSetStatus(1)"> <i class="fas fa-check-circle"></i>
				DUYỆT
			</button>
		</div>
	</div>
...
<script>
	...
	function fSetStatus(status){
		if (!confirm("Bạn có chắc chắn muốn thay đổi tình trạng?")){
			return;
		}
		$.ajax({
			url: "billDetail",
			type: "post",
			data: "id=${history.id}&status=" + status,
